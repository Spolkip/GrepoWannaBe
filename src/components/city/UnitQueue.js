...
const UnitQueueItem = ({ item, onCancel }) => {
    ...
    return (
        <div className="flex justify-between items-center bg-gray-600 p-2 rounded">
            <span className="font-semibold">{unit.name} ({item.amount})</span>
...
const UnitQueue = ({ unitQueue, onCancel }) => {
    if (!unitQueue || unitQueue.length === 0) {
        return (
            <div className="bg-gray-900 p-3 rounded-lg mb-4">
                <h4 className="text-lg font-semibold text-gray-400 text-center">Unit training queue is empty.</h4>
            </div>
        );
    }
    ...
    return (
        <div className="bg-gray-900 p-3 rounded-lg mb-4">
            <h4 className="text-lg font-semibold text-yellow-400 mb-2">Training Queue ({unitQueue.length}/5)</h4>
