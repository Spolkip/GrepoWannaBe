// src/components/city/WithdrawModal.js
import React, { useState } from 'react';
import unitConfig from '../../gameData/units.json';

const images = {};
const imageContext = require.context('../../images', false, /\.(png|jpe?g|svg)$/);
imageContext.keys().forEach((item) => {
    const key = item.replace('./', '');
    images[key] = imageContext(item);
});

const WithdrawModal = ({ city, onClose, onWithdraw }) => {
    const [withdrawAmounts, setWithdrawAmounts] = useState({});

    const handleAmountChange = (reinforcementId, unitId, value) => {
        const max = city.reinforcements[reinforcementId].units[unitId] || 0;
        const amount = Math.max(0, Math.min(max, parseInt(value, 10) || 0));
        setWithdrawAmounts(prev => ({
            ...prev,
            [reinforcementId]: {
                ...prev[reinforcementId],
                [unitId]: amount
            }
        }));
    };

    const handleWithdraw = () => {
        onWithdraw(city, withdrawAmounts);
        onClose();
    };

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-2xl border-2 border-gray-600 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-title text-3xl text-white">Withdraw Troops from {city.cityName}</h3>
                    <button onClick={onClose} className="text-gray-400 text-3xl leading-none hover:text-white">&times;</button>
                </div>
                <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                    {Object.values(city.reinforcements).map(reinforcement => (
                        <div key={reinforcement.id} className="bg-gray-700 p-3 rounded-lg">
                            <h4 className="font-bold text-lg text-yellow-400 mb-2">From: {reinforcement.originCityName} ({reinforcement.originOwnerUsername})</h4>
                            <div className="space-y-2">
                                {Object.entries(reinforcement.units).map(([unitId, count]) => {
                                    const unit = unitConfig[unitId];
                                    return (
                                        <div key={unitId} className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <img src={images[unit.image]} alt={unit.name} className="w-10 h-10 object-contain" />
                                                <div>
                                                    <p className="font-bold text-white">{unit.name}</p>
                                                    <p className="text-sm text-gray-400">Available: {count}</p>
                                                </div>
                                            </div>
                                            <input
                                                type="number"
                                                value={withdrawAmounts[reinforcement.id]?.[unitId] || ''}
                                                onChange={(e) => handleAmountChange(reinforcement.id, unitId, e.target.value)}
                                                className="bg-gray-800 text-white rounded p-2 w-24 text-center"
                                                placeholder="Amount"
                                                max={count}
                                                min="0"
                                            />
                                        </div>
                                    );
                                })}
                            </div>
                        </div>
                    ))}
                </div>
                <div className="mt-4 pt-4 border-t border-gray-600 flex justify-end">
                    <button onClick={handleWithdraw} className="btn btn-confirm py-2 px-6">
                        Withdraw Selected
                    </button>
                </div>
            </div>
        </div>
    );
};

export default WithdrawModal;
