...
const AcademyMenu = ({ cityGameState, onResearch, onClose }) => {
                ...
                <div className="overflow-y-auto space-y-4 pr-2">
                    {Object.entries(researchConfig).map(([id, config]) => {
                        ...
                        if (isResearched) {
                            ...
                            let reqText = `Requires Academy Lvl ${config.requirements.academy}`;
                            ...
                            button = <button onClick={() => onResearch(id)} disabled={!affordable} className={`btn ${affordable ? 'btn-upgrade' : 'btn-disabled'} w-full py-2 mt-2`}>Research</button>;
                        }

                        return (
                            <div key={id} className={`p-4 rounded-lg flex justify-between items-center ${isResearched ? 'bg-green-900/50' : 'bg-gray-700'}`}>
                                <div>
